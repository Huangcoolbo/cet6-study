...
原因：
- 它只处理已确认在项目边界内的 CET-6 内容
- 它的 `git add` 范围更收敛，不会像旧脚本那样默认把整个仓库都纳入提交
- 它的提交信息更接近项目语义，便于后续审查历史

`auto-push.ps1` 目前保留为**兼容包装脚本**，用于把旧入口转发到 `sync-cet6-study.ps1`。除非将来有明确的新职责，否则不应继续把两个脚本当成平行主入口维护。
...
- `COMMIT_MESSAGE_GUIDELINES.md` — 提交信息如何写得更具体，避免模糊描述
