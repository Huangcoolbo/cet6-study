...
- [ ] 评估是否要把 `CET6StudyAutoPush` 计划任务的动作从 `auto-push.ps1` 兼容包装层进一步收敛为直接调用 `sync-cet6-study.ps1`，减少一层历史兼容入口并降低后续漂移风险。
